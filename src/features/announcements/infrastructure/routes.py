...
"Anuncios" — el admin crea/publica, el empleado solo lee). El externo-
invitado NO tiene "Inicio" en la matriz de permisos (❌) y no aparece en su
navbar (docs/permisos-roles.md § Navbar por rol) — se rechaza aquí, en el
backend, no solo ocultando el ítem.

Un único `GET` sirve tanto a la tarjeta del dashboard (`empleado`, con
`limit`) como a la gestión del admin (sin filtrar por audiencia) — así lo
pidió el frontend para no duplicar contrato.
"""
