...
from .dependencies import get_login_with_google_use_case, get_refresh_session_use_case
...
from .schemas import AuthResponseDTO, LoginRequestDTO, TokenResponseDTO, UserDTO
...
def create_auth_router() -> APIRouter:
    ...
    async def login(
        ...
        result = await use_case.execute(dto.id_token)
    ...
    async def refresh(
        ...
        result = await use_case.execute(raw_cookie)
    ...
    async def logout(current_user: dict = Depends(get_current_user)):
        """Cierra sesión. Stateless en Fase 1: solo limpia la cookie del refresh token."""
